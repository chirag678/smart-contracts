...
contract Minter is ERC721, ERC721URIStorage, Pausable, Ownable {
    ...
    // store the waitlist NFTs
    WaitlistNFT[] private waitlist;

    // mapping of address to token id
    mapping(address => uint256) private addressToTokenId;

    // function to get tokenId of an address from mapping
    function getTokenURIByAddress(address _address) public view returns (string memory) {
        require(
            _address == waitlist[addressToTokenId[_address]].owner,
            "Minter: Address does not have a token"
        );
        return tokenURI(addressToTokenId[_address]);
    }

    // function to add a new NFT to the waitlist
    function addToWaitlist(string memory _name) private returns (uint) {
        // add the new NFT to the waitlist
        waitlist.push(WaitlistNFT(msg.sender, _name));
        //set token id
    ...
    function changeName(uint tokenId, string memory _name) public onlyHolder(tokenId) {
        // check if the NFT is in the waitlist
        require(tokenId < waitlist.length, "Invalid NFT ID");
        // change the name of the NFT
        waitlist[tokenId].name = _name;
    }
    ...
    function safeMint(address _to, string memory _name) public whenNotPaused {
        require(
            waitlist.length == 0 ||
                waitlist[addressToTokenId[_to]].owner != _to,
            "Only one token per address is allowed"
        );
        uint _tokenId = addToWaitlist(_name);
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override onlyHolder(tokenId) {
        // check if address from owns the token
        waitlist[tokenId].owner = to;
        ...
        returns (string memory)
    {
        ...
        string memory _imageURI = svgToImageURI(
            waitlist[_tokenId].owner,
            waitlist[_tokenId].name,
            _tokenId.toString()
        );
